...
{
    // Using : AppDomain.CurrentDomain.UnhandledException += MiniDumper.Current_UnhandledExceptionEventHandler;
    ...
    public sealed class MiniDumper
    {
        ...
        public static bool Write(string fileName, Typ dumpTyp)
        {
            using (var fs = new System.IO.FileStream(fileName, System.IO.FileMode.Create, System.IO.FileAccess.Write, System.IO.FileShare.None))
            {
                ...
                bool bRet = MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), fs.SafeFileHandle.DangerousGetHandle(), (uint)dumpTyp, ref exp, IntPtr.Zero, IntPtr.Zero);
                return bRet;
            }
        }
        
        public static void Current_UnhandledExceptionEventHandler(object sender, UnhandledExceptionEventArgs e)//System.StackOverflowException
        {
            var dateTime = DateTime.UtcNow;
            string fileName = string.Format("MINIDump_(UTCTime)_{0}-{1}-{2}_{3}{4}.dmp", dateTime.Year, dateTime.Month, dateTime.Day, dateTime.Hour, dateTime.Minute);
            Write(fileName);
        }
    }
}